...
outputs a prediction distribution Pr[S|Q=q] where S is the set of all possible sensitive values.  
...
2- For every distinct record (q,s), it makes $NB_Iterations prediction distributions while considering the full dataset 
   (i.e. while this record exists). It then takes their average in order to obtain a distribution P. 
   After removing this record from the dataset, it repeats the same process in 
   order to obtain a probability distribution P'. 
...
#
def compute_conditional_probabilities(q, counts, epsilon): 
	"""
	This method computes the probabilities Pr[s|$q] for all pairs ($q,s) whose 
	...
	for key, value in counts.iteritems():
		if q == key.split(';')[0]: 
			...
			term = 1.0 + min(max(0, value + noise), nb_records_q) #The addition of 1 is just for correction. 
...
def compute_proba_distribution(q, counts, epsilon, N, sa_values): 
	...
	for i in range(N):
		#compute all probabilities Pr[s|$q] for all SA s
		probabilities = dict()
		probabilities = compute_conditional_probabilities(q, counts, epsilon)
		#add the probabilities to the total summation
		for i, s in enumerate(sa_values):
			pred_proba[i] += probabilities[q+';'+s]
	return [x/float(N) for x in pred_proba] #normalize the summations
#
...
NB_Iterations = 50000 #number of predictions to make for each record
...
if __name__ == '__main__':
	...
	for xD in D:
		...
		fig = plt.gcf()
		filename = 'results/figures/cdf/%s/average/cdf_DP_%s_S%d_N%d_%s_average.pdf'%(Name,Name,Size,NB_Iterations,xD)
