"""
This file contains functions that read "Adult" dataset and the related generalization trees. 
...
CATEGORY_adult = [False, True, False, True, False, True, True,  True,  True,  True,  False,  False,  False,  True,  True]
#
#
def read_adult_data():
    """
    This method reads "Adult" dataset from 'data/adult.data'
    """
    nb_attributes = len(ATT_NAMES_adult)
    data, numeric_dict = [], []
    for i in range(nb_attributes):
        if CATEGORY_adult[i] is False:
    ...
    for line in data_file:
        ...
        data.append(temp)
        #keep a dictionary of numerical attributes
        index = 0
        for i in range(nb_attributes):
            if CATEGORY_adult[i] is False:
                try:
    ...
    for i in range(nb_attributes):
        if CATEGORY_adult[i] is False:
            static_file = open('data/adult_' + ATT_NAMES_adult[i] + '_static.pickle', 'wb')
...
def read_adult_tree(att_names):
    """
    This method reads, for every attribute in $att_names, the related tree from data/tree_*.txt, 
    and stores it in $att_trees.
    Moreover, it stores the the paths to leaves in the case of categorical attribute. 
    """
    prefix = 'data/adult_'
    att_trees = []
    paths_to_leaves = []
    for i in range(len(att_names)):
        if CATEGORY_adult[ATT_NAMES_adult.index(att_names[i])]:
            att_tree,  path_to_leaf = read_tree_file(att_names[i], prefix)
            att_trees.append(att_tree)
            paths_to_leaves.append(path_to_leaf)
        else:
            att_tree = read_pickle_file(att_names[i], prefix)
            att_trees.append(att_tree)
