...
the value of s for which the probability Pr[s|q] is maximal for every s in S. 
...
def compute_proba_distribution(q, counts, epsilon, N, sa_values): 
	...
	for i in range(N):
		...
		#get the maximal probability Pr[s|$q] among all pairs ($q, s) for all SA s
		maxx = 0.0
...
NB_Iterations = 10000 #number of predictions to make for each record
...
if __name__ == '__main__':
	...
	for xD in D:
		...
		fig = plt.gcf()
		filename = 'results/figures/cdf/%s/conditional/cdf_DP_%s_S%d_N%d_%s_conditional.pdf'%(Name,Name,Size,NB_Iterations,xD)
